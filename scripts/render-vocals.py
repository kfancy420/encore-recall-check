...
LINE_SECONDS = 4.8   # two bars at 100 BPM
TARGET = 4.15        # leave a breath before the next bar
...
LINES = ["Friday on the dock and the coffee's on me","Fuel at the pump, got a crew of three","Used to hold the paper till the end of the month","Now I open Spendly and I'm done with the hunt",*CHORUS,
 ...
 "No more digging through the truck for the slip","Tap, snap, submit, that's the whole trip",*CHORUS,"Expense it, don't stress it"]
...
for i,text in enumerate(LINES):
    ...
    d=render(text,175,f)
    target = TARGET if i < len(LINES)-1 else 2.4
    rate=max(140,min(320,int(175*d/target)))
